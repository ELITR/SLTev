...
def segemt_words_times(segment):
    """
...
def get_One_T(OStt, reference, aligns=None):
    ...
    for index in range(len(OStt)):
        ...
        just_words = []
        for i in sentence:
            segment_words_time = segemt_words_times(i)
        ...
        for i in reference[index][:-1]:
            ...
            t = just_times[int(p)] + (
                (just_times[int(p) + 1] - just_times[int(p)]) * (p - int(p))
            )
            ref_T[i] = t
            count += 1
            just_words.append(i)
        ...
        if aligns != None:
            max_value = 0
            align_dict = make_align_dict(aligns[index], reference[index])
            for i in just_words:
                try:
...
def build_A_time_based(sentence_segments):
    ...
    for segment in sentence_segments:
        ...
        for word in segment[3:-1]:
            if (
                word not in uniq_words_show_time.keys()
                and word in sentence_segments[-1][3:-1]
            ):
                uniq_words_show_time[word] = float(segment[0])
                uniq_words_estimate_time[word] = float(
                    float(segment[1]) + ((words.index(word) + 1) * step)
                )
...
def extract_match_words_based_on_time(estimat_times, display_times, start, end):
    """
...
def evaluate(Ts, MT, OStt):
    ...
    for j in range(len(OStt)):
        ...
        match_words = extract_match_words_based_on_time(
...
def evaluate_segmenter(Ts, MT, MovedWords, language, SLTev_home, temp_folder):
    ...
    segmenter_sentence, mWERQuality = segmenter(
        MT, Ts, language, SLTev_home, temp_folder
    )
